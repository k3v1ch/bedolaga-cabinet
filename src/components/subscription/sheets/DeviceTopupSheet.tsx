    ...
    >
      ...
      {devicePriceData?.available === false ? (
        ...
        <div className="space-y-4">
          ...
          {/* Откуда берётся цена: помесячная ставка, пропорционально остатку подписки */}
          {devicePriceData?.available &&
            devicePriceData.days_left !== undefined &&
            devicePriceData.base_device_price_kopeks !== undefined &&
            devicePriceData.base_device_price_kopeks > 0 && (
              <p className="text-center text-xs leading-relaxed text-dark-500">
                {t('subscriptionPage.devicePriceFormula', {
                  perMonth: devicePriceData.base_device_price_kopeks / 100,
                  days: devicePriceData.days_left,
                  currency: '₽',
                })}
              </p>
            )}
